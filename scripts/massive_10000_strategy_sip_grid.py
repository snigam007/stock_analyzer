#!/usr/bin/env python3
"""
scripts/massive_10000_strategy_sip_grid.py

Ultra-Massive 10,000-Strategy High-Throughput Quantitative Grid Explorer for Monthly SIP Investing.
Evaluates exactly 10,000 systematic SIP strategies across 4 multi-year historical regimes:
...
Total simulations: 10,000 strategies × 4 regimes = 40,000 full portfolio simulations.
Precomputes monthly rankings and asset prices for ultra-high throughput in-memory execution.
"""
import sys
import os
import math
...
def preload_and_precompute_sip_data():
    ...
    print("⏳ Preloading historical monthly prices & indicators for 10,000-strategy SIP grid...")
    ...
    for sym, group in df_all.groupby("symbol"):
        ...
        for m_date in monthly_dates:
            ...
            if m6 >= 25.0 and cp >= e50:
                candidates_by_filter_and_month["MOM_HURDLE_25"].setdefault(m_date, []).append({
                    "symbol": sym, "close": cp, "score": m6, "sector": sec
                })
            ...
            if m6 >= 35.0 and cp >= e50:
                candidates_by_filter_and_month["MOM_HURDLE_35"].setdefault(m_date, []).append({
                    "symbol": sym, "close": cp, "score": m6, "sector": sec
                })
...
def simulate_sip_strategy(
    ...
    monthly_sip_base: float = 100000.0
) -> dict:
    ...
    if booster == "FLAT_SIP_NO_DIP":
        stepup_pct = 0.0
    elif booster == "STEPUP_10_DIP_95":
        stepup_pct = 10.0
        enable_dip = True
    elif booster == "STEPUP_15_DIP_95_SKIM":
        stepup_pct = 15.0
        enable_dip = True
        enable_skim = True
    elif booster == "STEPUP_20_PYRAMID":
        stepup_pct = 20.0
        enable_pyramid = True
    elif booster == "STEPUP_10_LIQUID_SWEEP":
        stepup_pct = 10.0
        enable_dip = True
    ...
    for m_idx, m_date in enumerate(active_months):
        year_idx = m_idx // 12
        current_sip = monthly_sip_base * ((1.0 + stepup_pct / 100.0) ** year_idx)
        ...
        if enable_liquid_sweep and cash_reserve > 10000.0:
        ...
        for sym, pos in portfolio_holdings.items():
            ...
            if pos.get("asset_class") == "Stock":
                ...
                # Parabolic Skim booster
                if enable_skim and cur_gain_pct >= 120.0 and not pos.get("parabolic_skim_done", False):
                    pos["parabolic_skim_done"] = True
                    trim_shares = max(trim_shares, int(pos["shares"] * 0.08))
        ...
        # 2. Macro Hedge Allocation (if applicable)
        enable_macro = (asset_mode != "PURE_STOCKS")
        if enable_macro and not is_nifty_bull:
            gold_p = etf_map.get("GOLDBEES", {}).get(m_date, 70.0)
            hedge_pct = 0.10 if "90_10" in asset_mode else (0.20 if "80_20" in asset_mode else (0.15 if "BALANCED" in asset_mode else (0.05 if "95_5" in asset_mode else 0.30)))
            hedge_budget = cash_reserve * hedge_pct
        ...
        elif asset_mode == "MOMENTUM_ROTATE_DYNAMIC" and is_nifty_bull and "GOLDBEES" in portfolio_holdings:
            gold_p = etf_map.get("GOLDBEES", {}).get(m_date, 70.0)
            cash_reserve += portfolio_holdings["GOLDBEES"]["shares"] * gold_p
            del portfolio_holdings["GOLDBEES"]

        # 3. Tactical Dip-Buying booster: deploy cash reserve
        if enable_dip and not is_nifty_bull and cash_reserve > 10000.0:
            deploy_ratio = 0.95
        else:
            deploy_ratio = 1.0

        avail_for_equity = cash_reserve * deploy_ratio
        candidate_pool = candidates_by_filter_and_month.get(selection_filter, {}).get(m_date, [])

        # Pyramid into leading open winners if enabled
        if enable_pyramid and portfolio_holdings and avail_for_equity > 10000.0:
            top_winner = max(portfolio_holdings.items(), key=lambda x: (x[1]["peak_p"] - x[1]["entry_p"]) / x[1]["entry_p"] if x[1].get("asset_class") == "Stock" else -999)
            if top_winner[1].get("asset_class") == "Stock":
                sym_w = top_winner[0]
                s_rec = symbol_records.get(sym_w, {}).get(m_date)
                if s_rec:
                    cp = float(s_rec["close"])
                    pyr_alloc = avail_for_equity * 0.25
                    sh = int(pyr_alloc / cp)
                    if sh > 0:
                        cost = sh * cp
                        cash_reserve -= cost
                        avail_for_equity -= cost
                        top_winner[1]["shares"] += sh
                        top_winner[1]["cost"] += cost

        # Select New Equities from Precomputed Candidate Pool
        selected_candidates = []
        sector_counts = {}
...
# ─── 3. Build 10,000 Systematic SIP Strategies ────────────────────────────────
def build_10000_sip_strategies():
    strategies = []
    strat_idx = 1
    ...
    asset_modes = [
        ...
        "ASYMMETRIC_ALPHA_95_5",
        "DYNAMIC_REGIME_ROUTER"
    ] # 8 asset allocation modes
    ...
    boosters = [
        "FLAT_SIP_NO_DIP",
        "STEPUP_10_DIP_95",
        "STEPUP_15_DIP_95_SKIM",
        "STEPUP_20_PYRAMID",
        "STEPUP_10_LIQUID_SWEEP"
    ] # 5 compounding boosters
    # Total combinations = 8 * 10 * 5 * 5 * 5 = exactly 10,000 strategies!
        ...
        for sf in selection_filters:
            for bs in basket_sizes:
                for ep in exit_protocols:
                    for bst in boosters:
                        name = f"SIP_{strat_idx:05d}: {am[:10]} | {sf[:12]} | {bs}Stk | {ep[:10]} | {bst[:10]}"
                        strategies.append({
                            "id": f"SIP_{strat_idx:05d}",
                            "name": name,
                            "asset_mode": am,
                            "selection_filter": sf,
                            "basket_size": bs,
...
def main():
    print("=" * 125)
    print("🚀 ULTRA-MASSIVE 10,000-STRATEGY QUANTITATIVE EXPLORATION: MONTHLY SIP INVESTING")
    print("   Evaluating 10,000 Systematic Paradigms across 4 Diverse Historical Regimes")
    print("=" * 125)

    nifty_map, etf_map, symbol_records, candidates_by_filter_and_month, monthly_dates = preload_and_precompute_sip_data()
    strategies = build_10000_sip_strategies()
    ...
    results_table = []
    t_start = time.time()
    print(f"\n▶️ Launching SIP grid simulation (10,000 strategies × 4 regimes = 40,000 simulations)...")
    ...
    for i, strat in enumerate(strategies):
        ...
        if (i + 1) % 1000 == 0 or (i + 1) == len(strategies):
            ...
            print(f"   • Completed {i + 1:,}/{len(strategies):,} SIP strategies ({rate:.1f} strat/s | Elapsed: {time.time() - t_start:.1f}s)...")

    results_df = pd.DataFrame(results_table).sort_values("composite_score", ascending=False).reset_index(drop=True)

    out_csv = BASE_DIR / "scripts" / "sip_10000_strategies_results.csv"
    results_df.to_csv(out_csv, index=False)
    print(f"\n💾 Saved all 10,000 SIP strategy results to: {out_csv}")

    print("\n" + "=" * 125)
    print("🏆 TOP 10 ALL-TIME OPTIMUM SIP STRATEGIES (OUT OF 10,000 PERMUTATIONS)")
    print("=" * 125)
    cols = ["id", "composite_score", "full_xirr", "full_alpha", "bear_xirr", "bull_ret", "full_dd", "full_pr", "name"]
    print(results_df[cols].head(10).to_string(index=False))

    print("\n" + "=" * 125)
    print("🛡️ TOP 5 WORST BEAR-MARKET IMMUNE SIP STRATEGIES (HIGHEST BEAR XIRR)")
    print("=" * 125)
    bear_top = results_df.sort_values("bear_xirr", ascending=False).head(5)
    print(bear_top[cols].to_string(index=False))

    print("\n" + "=" * 125)
    print("🚀 TOP 5 BULL MARKET WEALTH MULTIPLIERS (HIGHEST 2.75Y XIRR)")
    print("=" * 125)
    bull_top = results_df.sort_values("full_xirr", ascending=False).head(5)
    print(bull_top[cols].to_string(index=False))

    print(f"\n✅ 10,000 SIP Strategies Grid Completed in {time.time() - t_start:.1f} seconds!")
